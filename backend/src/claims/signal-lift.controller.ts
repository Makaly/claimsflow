...
export class SignalLiftController {
  ...
  async getSignalLift(@Query('dateFrom') dateFrom?: string, @Query('dateTo') dateTo?: string) {
    ...
    const claims = await this.prisma.claim.findMany({
      where,
      select: { fraudSignals: true, labels: { select: { isFraud: true } } },
    });
    ...
    for (const claim of claims) {
      const signals: string[] = Array.isArray(claim.fraudSignals) ? (claim.fraudSignals as string[]) : [];
