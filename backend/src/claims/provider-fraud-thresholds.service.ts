...
export class ProviderFraudThresholdsService {
  ...
  async upsert(providerId: string, threshold: number, overriddenBy: string) {
    ...
    return this.prisma.providerFraudThreshold.upsert({
      where: { providerId },
      create: { providerId, threshold, fpRate: 0, fnRate: 0, overriddenBy, overriddenAt: new Date() },
      update: { threshold, overriddenBy, overriddenAt: new Date() },
    });
  }
  ...
  async recomputeMonthly() {
    ...
    const candidates = await (this.prisma.claim.groupBy as any)({
      by: ['providerId'],
    ...
    for (const c of candidates as Array<{ providerId: string }>) {
      ...
      const existing = await this.prisma.providerFraudThreshold.findUnique({ where: { providerId: c.providerId } });
    ...
    this.logger.log(`Fraud thresholds recomputed: ${updated} providers updated.`);
    return { updated };
  }

  private optimiseThreshold(labelled: Array<{ outcome: string; claim: { fraudSignals: any } }>) {
    const points = labelled.map(l => {
      const signals = Array.isArray(l.claim?.fraudSignals) ? l.claim.fraudSignals : [];
      return { score: Math.min(1, signals.length / 6), isFraud: l.outcome === 'fraud_confirmed' || l.outcome === 'rejected' };
    });
    let best = { threshold: GLOBAL_DEFAULT_HIGH, fpRate: 1, fnRate: 1, cost: Infinity };
    for (let t = 0.3; t <= 0.9; t += 0.05) {
      let fp = 0, fn = 0, tp = 0, tn = 0;
      for (const p of points) {
        const flagged = p.score >= t;
        if (flagged && !p.isFraud) fp++;
        else if (!flagged && p.isFraud) fn++;
        else if (flagged && p.isFraud) tp++;
        else tn++;
      }
